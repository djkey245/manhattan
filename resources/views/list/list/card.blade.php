...
<table class="table-bordered table">
        ...
        @elseif($item->actual > 49)
            @if(Auth::user()->actual == 2)
                ...
                @if($item->type == 'text' or $item->type == 'number' or $item->type == 'email' or $item->type == 'password' or $item->type == 'date')
                        ...
                        <th><input type="{{$item->type}}" class="form-control" value="{{$people[$item->name_eng]}}" id="add_{{$item->name_eng}}" maxlength="{{$item->max}}" minlength="{{$item->min}}" ></th></tr>
                @elseif($item->type == 'textarea')

                        <th><textarea minlength="{{$item->min}}"  maxlength="{{$item->max}}"  id="add_{{$item->name_eng}}" class="form-control">{{$people[$item->name_eng]}}</textarea></th></tr>
                @elseif($item->type == 'select')
    ...
    @endforeach
        <tr>
            ...
            <button onclick="cancel_hide('#list')" class="btn btn-primary btn-sm" id="button_cansel">Відмінити</button>
